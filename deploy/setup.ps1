param (
    $resourceBaseName="myBudget$( Get-Random -Maximum 1000)",
    $location='eastus'
)
...
dotnet build ..\src\MyBudget.Api\MyBudget.Api.csproj

Write-Host 'Packaging API project for deployment' -ForegroundColor Green
dotnet publish ..\src\MyBudget.Api\MyBudget.Api.csproj --self-contained -r win-x86 -o ..\publish
